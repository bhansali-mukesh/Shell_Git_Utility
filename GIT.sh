...
# Open Myself
alias Gitter="vi ${Git_File_Path}"

# Defines Workspace Directory 
export WORKSPACE="~/BitBucket/management-plane"
...
alias Git_Document='function MB() { Comment_Character="#"; Alias_Character="alias"; Git=""; Alias=""; while read line; do if [[ $line == $Comment_Character* ]]; then Git="$Git""\n""$line"; else Alias_Found=`echo "$line"| grep $Alias_Character`; if [[ M"$Alias_Found" != "M" ]]; then Alias=`echo "$line"| cut -d"=" -f1 | tr -s " "| cut -d" " -f2`; if [[ M"$Alias" != "M" ]]; then echo -e "\n$Alias" : "$Git"; Git=""; fi; else Git=""; fi; fi; done < "$Git_File_Path"; }; MB'
...
# Need Parameter      
# alias Create='function MBMB(){ Workspace; `Validator 1 "Need a Parameters ( Branch Name )" $@`; story=`echo $1| cut -d'_' -f1`; echo "alias $story=\"From=`Current`; git checkout $1 \"" >> $Git_File_Path; git checkout -b $1 --track $origin/$master; echo -e "\t\tType " \"$story\" " to Jump to this Branch, Anytime"; source $Git_File_Path; };MBMB'
...
# Now if I type "1745" on terminal Anytime, I am switched to this Branch
alias New='function MBMB(){ `Validator 1 "Need a Parameters ( Branch Name )" $@`; story=`echo $1| cut -d'_' -f1`; echo "alias $story=\"From=`Current`;git checkout $1 \"" >> $Git_File_Path; Master; Fetch; git checkout -b $1 $origin/$master; echo -e "\t\tType " \"$story\" " to Jump to this Branch, Anytime"; source $Git_File_Path; };MBMB'

# Checkout a Branch from a Tag/Build
alias Tag='function MBMB(){ `Validator 2 "Need 2 Parameters ( Branch Name, Tag Name )" $@`; story=`echo $1| cut -d'_' -f1`; echo "alias $story=\"From=`Current`;git checkout $1 \"" >> $Git_File_Path; Master; Fetch; git checkout -b $1 $2; echo -e "\t\tType " \"$story\" " to Jump to this Branch, Anytime"; source $Git_File_Path; };MBMB'
...
alias Remote='function MBMB(){ `Validator 1 "Need 1 Parameter ( Remote Branch Name )" $@`; story=`echo $1| cut -d'_' -f1`; echo "alias $story=\"From=`Current`; git checkout $1 \"" >> $Git_File_Path; Master; Fetch; git checkout -b $1 origin/"$1"; echo git checkout -b $1 origin/"$1"; echo -e "\t\tType " \"$story\" " to Jump to this Branch, Anytime"; source $Git_File_Path; };MBMB'
